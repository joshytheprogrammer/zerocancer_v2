<template>
  <div>
    ...
    <form @submit.prevent="subscribe" v-else class="waitlist-form block text-sm lg:flex text-center justify-center items-center lg:text-base">
      ...
      <button class="w-full mx-0 my-4 rounded-md bg-primary-600 text-white h-16 cursor-pointer px-8 py-4 lg:w-fit lg:rounded-tl-none lg:rounded-bl-none border-none" type="submit" :disabled="loading">Join</button>  
    </form>
  </div>
</template>
